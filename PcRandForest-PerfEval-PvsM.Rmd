...
# pred.table=NULL
 ...
 # 
 # cfit <- cforest(formPc, data=train.pm, ntree=1000, mtry = 5, control=ctree_control(mincriterion = 0, minsplit = 4L, minbucket = 2L))
...
x = foreach(i = 1:nrow(tune_settings), .combine = rbind) %dopar% { 
    library(randomForest)
    do_RF = randomForest(formPc, data = train.pm, test = test.pm, ntree = tune_settings$ntree[i], mtry = tune_settings$mtry[i])
