...
for (k in 1:length(IDs)){
  ...
  # does the classification match the population?
  ...
  pc_sum[k] <- sum(pc_pred != 0)
  pc_encID = rbind(pc_encID, unique(levels(temp$EncounterID))[k])
  }
pc_total = cbind.data.frame(pc_encID, pc_sum)

agg_temp <- aggregate(population~EncounterID, agg_data, length )

agg_total <- cbind.data.frame('EncounterID'=agg_temp[,1], 
                              'observed'=agg_temp[,2], 
                              'predicted'=pc_total[,2], 
                              'proportion true'=pc_total$pc_sum/agg_temp$population)
