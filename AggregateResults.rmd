...
today = 20190312
model = 'PMN'
thld = 50  # Equates to a threshold of the number of whistles that must be correctly classified to classify the encounter to the correct population. For example, 50 is 33% of 150 whistles, then 75 is 50% for pairwise classifiers.
...
for (k in 1:length(IDs)){
  ...
  correct <- data.frame("EncounterID" = IDs[k], "x"=sum(temp$CorrectDW > thld))
...
write.csv(agg_data_sum, file= paste('PcResults_AggregatedTestResults_MajorityVote', fn, sep=""), row.names = F )
