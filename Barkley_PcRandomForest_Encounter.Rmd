...
pcdata.P <- droplevels(filter(pcdata, pcdata$population == 'PEL'))
...
formPc_enc <- as.formula(paste("EncounterID ~ ", paste(names(pop.data)[7:length(enc_all)],collapse="+"))) #Use variables to classify to encounter
...
write.table(results.enc, file= paste('PcResults_Encounters_TotalResults', fn, sep=""), append = T, row.names = T, col.names=T, sep = ",")
