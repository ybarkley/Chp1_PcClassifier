...
levels(pcdata.Muncorr$EncounterID) <- c("M1", "M2", "M3", "M4" )
...
enc_all = NULL
for(i in c(NWHI)){
  enc_sub=droplevels(subset(pcdata.Nuncorr, pcdata.Nuncorr$group==i)) #selects all whistles from the randomly selected groups, drops empty levels too
...
formPc_enc <- as.formula(paste("EncounterID ~ ", paste(names(pcdata.Nuncorr)[7:length(enc_all)],collapse="+"))) #Use variables to classify to encounter
...
```{r}
write.table(results.enc, "C:\\Users\\yvers\\OneDrive\\PHD\\CHP1-FKW\\data\\results\\2018\\20181213_ENCOUNTERS/PcResults_Encounters_TotalResults_NWHI_20181213.csv", append = T, row.names = T, col.names=T, sep = ",")

write.table(varIMPtotal.enc, 'C:\\Users\\yvers\\OneDrive\\PHD\\CHP1-FKW\\data\\results\\2018\\20181213_ENCOUNTERS/PcResults_Encounters_NWHI_ImpVar_20181213.csv', sep=",", append=TRUE)
