...
ks_vars_m <- pcdata.M[, c(7:24, 28:32, 43:47, 49:53)]
ks_vars_n <- pcdata.N[, c(7:24, 28:32, 43:47, 49:53)]
ks_vars_p <- pcdata.P[, c(7:24, 28:32, 43:47, 49:53)]
...
for (m in colnames(ks_vars_m))
  ...
  #for (n in colnames(ks_vars_n))
     #{
       ks <- ks.test(ks_vars_m[,m], ks_vars_n[,m])
       
      ks_df <- cbind(colnames(ks_vars_m[m]),ks$statistic,ks$p.value)

      ks_resultsALL <- rbind(ks_resultsALL, ks_df)
    
       }
  colnames(ks_resultsALL) <- c("variable", "statistic", "p.value")   
