...
#6.
  # Important Variables
  varIMPtemp.pn <- as.data.frame(fit.pn$importance)
  varIMP.pn <- varIMPtemp.pn[order(varIMPtemp.pn$MeanDecreaseGini), , drop =F]
  output_varIMPtotal.pn <- cbind(rep, rbind(output_varIMPtotal.pn, varIMP.pn))
...
write.table(output_CorrectClass.pn, "data/results/PcResults_PN_ConfusionMatrix_20181209.csv", append = T, row.names = T, col.names=T, sep = ",")

write.table(output_results_pn, "data/results/PcResults_PN_TotalResults_20181209.csv", append = T, row.names = T, col.names=T, sep = ",")

write.table(output_WhistleClass.pn, "data/results/PcResults_PN_TotalWhistleClass_20181209.csv", append = T, row.names = F, col.names=T, sep = ",")

write.table(output_TrainSetAll.pn, "data/results/PcResults_PN_TrainSets_20181209.csv", append = T, col.names=T, sep = ",")

write.table(output_TestSetAll.pn, "data/results/PcResults_PN_TestResults_20181209.csv", append = T, col.names=T, sep = ",")

write.table(output_varIMPtotal.pn, "data/results/PcResults_PN_ImportantVars_20181209.csv", append = T, col.names=T, sep = ",")

write.table(tunedAll.pn, "data/results/PcResults_PN_TunedParameters_20181209.csv", append = T, col.names=T, sep = ",")

write.table(output_ModelFit.pn, "data/results/PcResults_PN_TotalModels_20181209.csv", append = T, col.names=F, sep = ",")

#9) Save encounter classifications based on majority of whistles
write.table(pc_classPN, "data/results/PcResults_PN_EncounterClass_20190325.csv", append = F, col.names=T, row.names = F, sep = ",")
